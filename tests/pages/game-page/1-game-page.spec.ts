...
test('should be able to play a game round with 2 players', async ({ page: firstPlayerPage }) => {
    ...
    await addCardAsFood(firstPlayerPage, 1, 0, 4, 'Long neck', 4)
    ...
    await addSpeciesToTheLeft(firstPlayerPage, 1, 3, 'Fertile')
    await increaseSpeciesSize(firstPlayerPage, 1, 2, 2, 'Forager')
    await increaseSpeciesPopulation(firstPlayerPage, 1, 2, 1, 'Carnivore')
    await finishTurnEvolvingAndWaitForOthersToEvolve(firstPlayerPage)
    await checkOpponentSpecies(secondPlayerPage, 0, 0, 1, 1)
    await assertNumberOfFoodInWaterPlan(firstPlayerPage, 0)
    await assertNumberOfFoodInWaterPlan(secondPlayerPage, 0)

    await addCardAsFood(secondPlayerPage, 1, 1, 4, 'Carnivore', 1)
    await assertNumberOfHiddenFood(firstPlayerPage, 2)

    await addSpeciesToTheRight(secondPlayerPage, 1, 3, 'Fertile')
    await addSpeciesFeature(secondPlayerPage, 1, 2, 2, 'Long neck')
